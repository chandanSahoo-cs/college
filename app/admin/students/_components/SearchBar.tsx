...
export default function SearchBar({ initialStudents, total, totalPages, currentPage }: SearchBarProps) {
 ...
 if(total === 0){
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    setIsSearching(true)
    try {
      const { data, total, totalPages } = await searchStudents(searchTerm, 1, 10)
      setStudents(data)
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        const { data, total, totalPages } = await searchStudents(searchTerm, currentPageState, 10)
        setStudents(data)
        setTotalState(total)
        setTotalPagesState(totalPages)
      } else {
